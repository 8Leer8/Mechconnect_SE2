import { StyleSheet } from 'react-native';

export const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#111214' },
  loader: { marginTop: 100 },
  scroll: { flex: 1 },
  scrollContent: { paddingBottom: 24 },
  header: {
    backgroundColor: '#1A1C1E',
    paddingTop: 56,
    paddingBottom: 20,
    alignItems: 'center',
    borderBottomWidth: 1,
    borderBottomColor: '#2A2C2E',
  },
  avatarPlaceholder: {
    width: 72,
    height: 72,
    borderRadius: 36,
    backgroundColor: '#2A2C2E',
    justifyContent: 'center',
    alignItems: 'center',
    marginBottom: 12,
  },
  name: { fontSize: 20, fontWeight: 'bold', color: '#fff', marginBottom: 4 },
  subtitle: { fontSize: 12, color: '#8E8E93' },
  section: { marginTop: 20, paddingHorizontal: 16 },
  sectionRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 },
  sectionTitle: { fontSize: 16, fontWeight: '600', color: '#fff' },
  addBtn: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 6,
    paddingVertical: 6,
    paddingHorizontal: 12,
    borderRadius: 8,
    borderWidth: 1,
    borderColor: '#FF8C00',
  },
  addBtnText: { color: '#FF8C00', fontSize: 14, fontWeight: '600' },
  emptyCard: {
    backgroundColor: '#1A1C1E',
    borderRadius: 14,
    padding: 24,
    alignItems: 'center',
    borderWidth: 1,
    borderColor: '#2A2C2E',
  },
  emptyText: { color: '#8E8E93', fontSize: 15, marginTop: 10 },
  emptySubtext: { color: '#555', fontSize: 13, marginTop: 4 },
  serviceCard: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
    backgroundColor: '#1A1C1E',
    borderRadius: 14,
    padding: 14,
    marginBottom: 8,
    borderWidth: 1,
    borderColor: '#2A2C2E',
  },
  addonGroupCard: {
    backgroundColor: '#1A1C1E',
    borderRadius: 14,
    padding: 14,
    marginBottom: 10,
    borderWidth: 1,
    borderColor: '#2A2C2E',
  },
  addonGroupHeader: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
    gap: 12,
    marginBottom: 12,
  },
  addonGroupHeaderText: {
    flex: 1,
  },
  addonGroupTitle: { fontSize: 16, fontWeight: '700', color: '#fff' },
  addonGroupSubtitle: { fontSize: 12, color: '#8E8E93', marginTop: 3 },
  addonEmptyCard: {
    backgroundColor: '#222426',
    borderRadius: 12,
    padding: 18,
    alignItems: 'center',
    borderWidth: 1,
    borderColor: '#2A2C2E',
  },
  addonItemCard: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
    backgroundColor: '#222426',
    borderRadius: 12,
    padding: 12,
    marginBottom: 8,
    borderWidth: 1,
    borderColor: '#2A2C2E',
  },
  specialtyCard: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
    backgroundColor: '#1A1C1E',
    borderRadius: 14,
    padding: 14,
    marginBottom: 8,
    borderWidth: 1,
    borderColor: '#2A2C2E',
  },
  serviceInfo: { flex: 1 },
  serviceName: { fontSize: 16, fontWeight: '600', color: '#fff' },
  servicePrice: { fontSize: 14, color: '#FF8C00', marginTop: 2 },
  specialtyDesc: { fontSize: 13, color: '#8E8E93', marginTop: 2 },
  specialtyMetaRow: {
    marginTop: 8,
    flexDirection: 'row',
    alignItems: 'center',
    gap: 8,
    flexWrap: 'wrap',
  },
  specialtyMetaText: { fontSize: 12, color: '#8E8E93' },
  pendingBadge: {
    borderRadius: 999,
    paddingHorizontal: 10,
    paddingVertical: 4,
    borderWidth: 1,
    borderColor: '#FF8C00',
    backgroundColor: 'rgba(255, 140, 0, 0.14)',
  },
  pendingBadgeText: {
    color: '#FFB357',
    fontSize: 11,
    fontWeight: '700',
    textTransform: 'uppercase',
  },
  approvedBadge: {
    borderRadius: 999,
    paddingHorizontal: 10,
    paddingVertical: 4,
    borderWidth: 1,
    borderColor: '#22C55E',
    backgroundColor: 'rgba(34, 197, 94, 0.14)',
  },
  approvedBadgeText: {
    color: '#86EFAC',
    fontSize: 11,
    fontWeight: '700',
    textTransform: 'uppercase',
  },
  rejectedBadge: {
    borderRadius: 999,
    paddingHorizontal: 10,
    paddingVertical: 4,
    borderWidth: 1,
    borderColor: '#EF4444',
    backgroundColor: 'rgba(239, 68, 68, 0.14)',
  },
  rejectedBadgeText: {
    color: '#FCA5A5',
    fontSize: 11,
    fontWeight: '700',
    textTransform: 'uppercase',
  },
  serviceActions: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 8,
  },
  editBtn: { padding: 8 },
  removeBtn: { padding: 8 },
  menuItem: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 12,
    backgroundColor: '#1A1C1E',
    padding: 14,
    borderRadius: 14,
    marginBottom: 8,
    borderWidth: 1,
    borderColor: '#2A2C2E',
  },
  menuItemContent: {
    flex: 1,
  },
  menuText: { flex: 1, fontSize: 15, color: '#fff' },
  menuSubtext: { fontSize: 12, color: '#8E8E93', marginTop: 2 },
  logoutItem: { borderColor: '#2A2C2E' },
  logoutText: { color: '#FF3B30' },
  modalOverlay: {
    flex: 1,
    backgroundColor: 'rgba(0,0,0,0.5)',
    justifyContent: 'flex-end',
  },
  modalBox: {
    backgroundColor: '#1A1C1E',
    borderTopLeftRadius: 16,
    borderTopRightRadius: 16,
    maxHeight: '70%',
    paddingBottom: 20,
  },
  addonModalBox: {
    maxHeight: '88%',
    paddingBottom: 10,
  },
  modalHeader: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
    padding: 16,
    borderBottomWidth: 1,
    borderBottomColor: '#2A2C2E',
  },
  modalTitle: { fontSize: 18, fontWeight: 'bold', color: '#fff' },
  modalScroll: { maxHeight: 320, padding: 16 },
  modalEmpty: { color: '#8E8E93', textAlign: 'center', padding: 24 },
  addonModalScroll: { flex: 1 },
  addonModalContent: { padding: 16, paddingBottom: 28 },
  addonServiceList: {
    gap: 8,
  },
  availableRow: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
    paddingVertical: 14,
    paddingHorizontal: 16,
    borderRadius: 8,
    marginBottom: 8,
    backgroundColor: '#222426',
    borderWidth: 1,
    borderColor: '#2A2C2E',
  },
  addonServiceSelected: {
    borderColor: '#FF8C00',
    backgroundColor: 'rgba(255, 140, 0, 0.12)',
  },
  availableInfo: { flex: 1 },
  availableName: { fontSize: 16, fontWeight: '600', color: '#ECEDEE', marginBottom: 4 },
  availableDesc: { fontSize: 13, color: '#8E8E93' },
  backBtn: {
    padding: 4,
  },
  priceContent: {
    padding: 20,
  },
  addonServiceField: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
    gap: 12,
    backgroundColor: '#222426',
    borderRadius: 10,
    borderWidth: 1,
    borderColor: '#FF8C00',
    paddingHorizontal: 14,
    paddingVertical: 12,
  },
  addonServiceText: {
    flex: 1,
    color: '#fff',
    fontSize: 14,
    fontWeight: '600',
    paddingRight: 12,
  },
  addonServicePlaceholder: {
    flex: 1,
    color: '#8E8E93',
    fontSize: 14,
    fontWeight: '500',
    paddingRight: 12,
  },
  addonTextInput: {
    backgroundColor: '#222426',
    borderRadius: 10,
    borderWidth: 1,
    borderColor: '#2A2C2E',
    color: '#fff',
    fontSize: 14,
    paddingHorizontal: 12,
    paddingVertical: 12,
  },
  addonTextArea: {
    borderRadius: 10,
    borderWidth: 1,
    borderColor: '#2A2C2E',
    backgroundColor: '#222426',
    color: '#fff',
    fontSize: 14,
    minHeight: 96,
    paddingHorizontal: 12,
    paddingVertical: 10,
  },
  serviceDetailCard: {
    backgroundColor: '#222426',
    borderRadius: 14,
    padding: 16,
    marginBottom: 24,
    borderWidth: 1,
    borderColor: '#2A2C2E',
  },
  serviceDetailName: {
    fontSize: 18,
    fontWeight: '600',
    color: '#fff',
    marginBottom: 6,
  },
  serviceDetailInfo: {
    fontSize: 14,
    color: '#8E8E93',
  },
  priceInputSection: {
    marginBottom: 24,
  },
  priceLabel: {
    fontSize: 15,
    fontWeight: '600',
    color: '#fff',
    marginBottom: 10,
  },
  priceInputWrapper: {
    flexDirection: 'row',
    alignItems: 'center',
    backgroundColor: '#222426',
    borderRadius: 10,
    borderWidth: 2,
    borderColor: '#FF8C00',
    paddingHorizontal: 16,
    paddingVertical: 4,
  },
  currencySymbol: {
    fontSize: 24,
    fontWeight: '600',
    color: '#FF8C00',
    marginRight: 8,
  },
  priceInput: {
    flex: 1,
    fontSize: 24,
    fontWeight: '600',
    color: '#fff',
    paddingVertical: 12,
  },
  priceHint: {
    fontSize: 12,
    color: '#8E8E93',
    marginTop: 8,
  },
  addServiceBtn: {
    backgroundColor: '#FF8C00',
    borderRadius: 10,
    paddingVertical: 16,
    alignItems: 'center',
    justifyContent: 'center',
    shadowColor: '#FF8C00',
    shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 0.3,
    shadowRadius: 4,
    elevation: 4,
  },
  addServiceBtnDisabled: {
    opacity: 0.6,
  },
  addServiceBtnText: {
    fontSize: 16,
    fontWeight: '700',
    color: '#fff',
  },
  proofContent: {
    padding: 16,
    paddingBottom: 22,
  },
  proofSection: {
    marginBottom: 16,
  },
  proofLabel: {
    color: '#ECEDEE',
    fontSize: 14,
    fontWeight: '600',
    marginBottom: 8,
  },
  sourceChipWrap: {
    flexDirection: 'row',
    flexWrap: 'wrap',
    gap: 8,
  },
  sourceChip: {
    borderRadius: 999,
    borderWidth: 1,
    borderColor: '#2A2C2E',
    backgroundColor: '#222426',
    paddingHorizontal: 12,
    paddingVertical: 7,
  },
  sourceChipActive: {
    borderColor: '#FF8C00',
    backgroundColor: 'rgba(255, 140, 0, 0.16)',
  },
  sourceChipText: {
    color: '#A0A4AA',
    fontSize: 12,
    fontWeight: '600',
  },
  sourceChipTextActive: {
    color: '#FFC375',
  },
  proofTextArea: {
    borderRadius: 10,
    borderWidth: 1,
    borderColor: '#2A2C2E',
    backgroundColor: '#222426',
    color: '#fff',
    fontSize: 14,
    minHeight: 108,
    paddingHorizontal: 12,
    paddingVertical: 10,
  },
  proofFileBtn: {
    borderRadius: 10,
    borderWidth: 1,
    borderColor: '#FF8C00',
    backgroundColor: 'rgba(255, 140, 0, 0.12)',
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
    gap: 8,
    paddingVertical: 12,
  },
  proofFileBtnText: {
    color: '#FFB357',
    fontWeight: '700',
    fontSize: 13,
  },
  proofFileMeta: {
    marginTop: 8,
    color: '#8E8E93',
    fontSize: 12,
  },
  rejectedNoteCard: {
    borderRadius: 10,
    borderWidth: 1,
    borderColor: '#EF4444',
    backgroundColor: 'rgba(239, 68, 68, 0.12)',
    paddingHorizontal: 12,
    paddingVertical: 10,
    marginBottom: 16,
  },
  rejectedNoteLabel: {
    color: '#FCA5A5',
    fontSize: 12,
    fontWeight: '700',
    marginBottom: 4,
    textTransform: 'uppercase',
  },
  rejectedNoteText: {
    color: '#FEE2E2',
    fontSize: 13,
    lineHeight: 18,
  },
});
